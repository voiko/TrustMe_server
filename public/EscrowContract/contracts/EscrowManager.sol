...
contract EscrowManager {
    ...
    function setAgreement(uint256 _index) public payable {
       ...
       emit ViewTrade(
            _index,
            uint256(trades[_index]._state),
            address(trades[_index]._escrow),
            trades[_index]._escrow.getBalance(),
            trades[_index]._escrow.seller(),
            trades[_index]._escrow.buyer(),
            trades[_index]._sellerPaid,
            trades[_index]._buyerPaid,
            trades[_index].sellerConfirmed,
            trades[_index].buyerConfirmed
        );
    }
